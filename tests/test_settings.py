...
class TestLocation:
    def test_linux_follows_xdg_config_home(self, tmp_path):
        assert config_directory(
            environ={"XDG_CONFIG_HOME": str(tmp_path / "somewhere")},
            home=tmp_path,
            platform="linux",
        ) == (tmp_path / "somewhere" / "breadsched")

    def test_linux_falls_back_to_dot_config(self, tmp_path):
        assert config_directory(environ={}, home=tmp_path, platform="linux") == (
            tmp_path / ".config" / "breadsched"
        )
